...
## Pre-Installation
There are two different ways to install Windows, it depends on personal preference.
...
If you followed the English (World) guide, you will see an error while opening the OOBE called `OOBEREGION`. The reason it does this is due to the fact World is not a valid region, and does not technically exist. Skipping is the only way to get past this.
